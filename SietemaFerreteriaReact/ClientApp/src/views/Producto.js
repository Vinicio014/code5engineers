        ...
        <>
            <Card>
                ...
                <CardBody>
                    ...
                    {/* Botón para alternar entre activos y no activos */}
                    <Button color="warning" size="sm" className="ml-2" onClick={() => setMostrarActivos(!mostrarActivos)}>
                        {mostrarActivos ? 'Ver No Activos' : 'Ver Activos'}
                    </Button>
                    <hr></hr>
                    <DataTable
                        columns={columns}
                        data={productos.filter(prod => prod.esActivo === mostrarActivos)} 
                            ...
                            <FormGroup>
                                ...
                                <Input bsSize="sm" type={"select"} name="idCategoria" onChange={handleChange} value={producto.idCategoria} >
                                    <option value={0}>Seleccionar</option>
                                    {
                                        categorias.map((item) => (
                                            item.esActivo &&
                                            <option key={item.idCategoria} value={item.idCategoria}>{item.descripcion}</option>
                                        ))
                                    }
                                </Input>
                            </FormGroup>
